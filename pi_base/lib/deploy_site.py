...
class DeploySiteDB:
    ...
    def __init__(self, conf_file=None, db_file=None, config_paths=None, secrets_paths=None, loggr=logger, debug=False):  # noqa: PLR0913
        ...
        if gd_secrets:
            ...
            self.gds, extras = gd_connect(self.loggr, gd_secrets, {
                'gd_sites_folder_id': None,
                'gd_sites_file_title': None
    ...
    def db_file_save_fd(self, sites, out_file_fd):
        ...
        for site in sites:
            ...
            for c in self.db_file_cols:
                ...
                key = c.replace(' ', '_')
                row += [site[key] if key in site else '']
    ...
    def add(self, name: str, site: DeploySite) -> int: pass
    def get(self, name: str) -> Tuple[int, DeploySite]: pass
    def delete(self, name: str) -> int: pass
    def update(self, name: str, site: DeploySite) -> int: pass
...
def cmd_sites(db, args):
    ...
    for site in db.sites:
        ...
        for c in db.cols + db.cols_optional:
            key = c.replace(' ', '_')
            if show_secret or not key in db.cols_secret:
                vals += [site[key]]
        print(', '.join(vals))
    return 0
