...
Login either in RPi console or via SSH as ssh://pi@RPI.local. If you entered a different hostname and/or userbname in the Raspberry Pi Imager, use that hostname.
...
For details on creating new Service Account and setting up ``*_secrets.json`` key file (properties for Google APIs) see `GD_SERVICE.md <./lib/GD_SERVICE.md>`__. After creating the key file, add all needed ``pibase_XXX`` properties to it.
...
-  Install Raspberry Pi OS on a fast 4~16GB Micro-SD Card, boot RPi with it
   ...
   -  Used latest 2022-04-04-raspios-bullseye-armhf.img.xz (2022-0720) https://www.raspberrypi.com/software/operating-systems/
   -  Used Raspberry Pi Imager https://www.raspberrypi.com/software/
...
This project settled on `cliff <https://pypi.org/project/cliff/>`__ for arguments parsing, and argparse in some instances. Other CLI tools of interest:
...
Lightdm details: https://wiki.archlinux.org/title/LightDM#Enabling_autologin
